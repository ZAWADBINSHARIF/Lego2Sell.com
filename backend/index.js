...
const userRoute = require("./routes/order.js");
const Joi = require("joi");
const passwordComplexity = require("joi-password-complexity");
const GetQuote = require("./models/GetQuote.js");
const Getorder = require("./models/Getorder.js");
...
const legoRoute = require("./routes/lego.js");
...
const SearchItem = require("./models/Search.js");
const User = require("./models/user.model.js");
const fetch = require("node-fetch");
const CryptoJS = require('crypto-js');
const { createSuccessResponse, createErrorResponse } = require("./utils/responseHelper.js");
...
cloudinary.config({
  ...
  api_key: '286136878258575',
  api_secret: 'lSqm4taJ4SuXCqptj7LT1Cpt_Oo'
});
...
const keysecret = Config.JWTSECRET.SECRET;
...
const allowedOrigins = [
  ...
  'https://lego2sell.com',
  // "https://lego-aqib.vercel.app",
  // "https://front-end-lego-henna.vercel.app",
  // "https://front-end-rho-snowy.vercel.app",
...
app.post('/upload', async (req, res) => {
  ...
  try {
    ...
    const imageUrl = await uploadImage(uploadedImage.path);
...
app.put("/update-email/:userId", async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Email updated successfully", token: newToken });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Internal Server Error" });
  }
...
app.post("/login", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Login successfully", userId: user._id, token: token });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Internal Server Error" });
  }
  ...
  try {
    ...
    if (setusertoken) {
      const mailOptions = {
        ...
        text: `This Link Valid For 2 MINUTES https://lego2sell.com/forgotpassword/${userfind.id}/${userfind.verifytoken}`,
        html: `<div style="word-spacing:normal;background-color:#eeeeee">
  <div style="background-color:#eeeeee">
...
app.post("/forgotpassword/:id/:token", async (req, res) => {
  ...
  console.log("id, token", id, token, password);
  const encryptedData = req.header('source');
  // Encryption key (must match the key used for encryption)
  const encryptionKey = 'legotwosell';
...
app.delete("/deleteUser/:id", async (req, res) => {
  const { id } = req.params;
  console.log(id);
  const resp = await UserData.deleteOne({
    _id: id
  });
  console.log(resp);
  res.send(createSuccessResponse(resp));
});
